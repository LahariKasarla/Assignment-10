...
Original file is located at
    https://colab.research.google.com/drive/1hPVQrkw4x456FCQaiZVYGBzPkJjHAcVK
"""
...
for i in range(250):
    ...
    for x in range(100):
        ...
        if dice <= 2 :
            ...
            step = step + randint(1,7)
        
    print(step)

#Q2)Data Generation 

#(a) for Multiple Regression
...
x = np.asarray(df)
eps= scipy.stats.norm.rvs(0,0.25,250)
y = (0.16*x) +eps
print(y)
type(y)
print(np.size(y))
...
# %matplotlib inline 
y_true = df.Y.values.copy() #actual
y_predicted = results.predict(df.X) 
y_population = 0.16*df.X.values 
...
for i in range(epochs):
  ...
  d1 = round((-2/n) * np.sum(X * (y - y_p)) + (lam *b1))
  d0 = (-2/n) * np.sum(y - y_p)
  b1 = b1 - (l*d1)
  b0 = b0 - (l*d0)
